...
def _gaussian_kernel_2d(channel:int, kernels_size:Union[int, Sequence[int]], sigma: Union[float,Sequence[float]]):
    kernel_x: Tensor, kernel_y: Tensor = _gaussian_kernel(kernel_size[0],sigma[0]),/_gaussian_kernel(kernel_size[1],sigma[1])
...
class StructuredSimilarityIndexMeasure2d(Metric):
    ...
    def __call__(self, pred: Tensor, target: Tensor):
        ...
        kernel_size = 2*[kernel_size] if not gaussian else  2*[2*int
        ...
        output_list = torch.conv2d( preds.cat(target,preds*preds, targets*targets,preds*targets),kernel,groupps=channel)
